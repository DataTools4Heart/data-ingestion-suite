...
docker run -d \
    ...
    --env APP_CONF_FILE=/usr/local/tofhir/conf/docker/deployment/engine-only/tofhir-engine.conf \
    --env LOGBACK_CONF_FILE=/usr/local/tofhir/conf/docker/deployment/engine-only/logback.conf \
